...
//标量乘积
Point3D & Point3D::scalar(float rhs)
{
	Point3D _point(this->x * rhs, this->y * rhs, this->z * rhs);
	return _point;
}
////重载操作符=
//Point3D& Point3D::operator=(Point3D& rhs)
...
/*------------------------Polygon3D相关函数------------------------*/
//Polygon3D初始化函数
Polygon3D::Polygon3D() :_mData(NULL), _size(0) {};
...
{
	Point3D ab = b - a;
	float t = (splitPlane.d - dot(splitPlane.n, a)) / dot(splitPlane.n, ab);
	assert(t >= 0.0f&&t <= 1.0f);
	Point3D p = a + ab.scalar(t);
	return p;
}
...
int const classifyPointToPlane(Point3D& p, Plane& splitPlane)
{
	...
	float dist = dot(p, splitPlane.n) - splitPlane.d;
